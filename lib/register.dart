...
class _RegisterState extends State<Register> {
  ...
  // Add profile with user's UID as document ID
  Future<void> addProfile(Profile profile, String uid) async {
    await profileCollection.doc(uid).set(profile.toJson());
    print("Profile added successfully!");
  }

  // Check if a profile with the same email already exists
  Future<bool> doesProfileExist(String email) async {
    QuerySnapshot snapshot = await profileCollection.where('email', isEqualTo: email).get();
  ...
  void uploadProfile(String uid) async {
    Profile profile = Profile(
      id: uid, // Set the UID as the document ID
      email: emailCon.text,
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    if (emailCon.text.isNotEmpty &&
                      ...
                      try {
                        // Check if a profile with the same email already exists
                        bool exists = await doesProfileExist(emailCon.text.trim());
                        if (exists) {
                        ...
                        // Upload profile with the UID as the document ID
                        uploadProfile(uid);
